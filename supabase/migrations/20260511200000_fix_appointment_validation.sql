...
BEGIN
    -- Convert start time to tutor's local time to get day of week and time
    -- NOTE: This assumes a fixed timezone for simplicity. A real-world app would use dynamic timezones.
    v_day_of_week := to_char(p_start_time, 'Day'); 
    ...
    INSERT INTO public.appointments (student_id, tutor_id, start_time, end_time, status)
    VALUES (p_student_id, p_tutor_id, p_start_time, p_end_time, 'Scheduled')
    RETURNING public.appointments.appointment_id INTO appointment_id;
...
WHERE NOT EXISTS (
    ...
    WHERE wh.tutor_id = a.tutor_id
      AND wh.day_of_week = to_char(a.start_time, 'Day')
      AND wh.is_working = TRUE
